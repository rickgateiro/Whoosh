# Version: 4.0
# Date: 2024-03-05
# Changes: Added visualization tab with file content preview functionality
...
class PDFProcessorGUI:
    ...
    def setup_search_tab(self):
        ...
        # JSON Files List
        ttk.Label(files_frame, text="Arquivos JSON:").pack(anchor=W, padx=5, pady=2)
        self.json_listbox = ttk.Treeview(files_frame, selectmode="extended", show="tree", height=10)
        self.json_listbox.pack(fill=X, padx=5, pady=5)
        
        # TXT Files List
        ttk.Label(files_frame, text="Arquivos TXT:").pack(anchor=W, padx=5, pady=2)
        self.txt_listbox = ttk.Treeview(files_frame, selectmode="extended", show="tree", height=10)
        self.txt_listbox.pack(fill=X, padx=5, pady=5)
        
        # Refresh button
        ttk.Button(files_frame, text="Atualizar Listas", 
                  command=self.refresh_file_lists).pack(pady=5)
        ...
        # Results area
        results_frame = ttk.LabelFrame(search_container, text="Resultados", padding=10)
    ...
    def setup_view_tab(self):
        ...
        # JSON Files List
        ttk.Label(files_frame, text="Arquivos JSON:").pack(anchor=W, padx=5, pady=2)
        self.view_json_listbox = ttk.Treeview(files_frame, selectmode="browse", show="tree", height=10)
        self.view_json_listbox.pack(fill=X, padx=5, pady=5)
        self.view_json_listbox.bind('<<TreeviewSelect>>', self.on_file_select)
        
        # TXT Files List
        ttk.Label(files_frame, text="Arquivos TXT:").pack(anchor=W, padx=5, pady=2)
        self.view_txt_listbox = ttk.Treeview(files_frame, selectmode="browse", show="tree", height=10)
        self.view_txt_listbox.pack(fill=X, padx=5, pady=5)
        self.view_txt_listbox.bind('<<TreeviewSelect>>', self.on_file_select)
        
        # Refresh button
        ttk.Button(files_frame, text="Atualizar Listas", 
                  command=self.refresh_view_lists).pack(pady=5)
    ...
    def refresh_file_lists(self):
        ...
        self.json_listbox.delete(*self.json_listbox.get_children())
        self.txt_listbox.delete(*self.txt_listbox.get_children())
        
        directory = self.dir_entry.get()
        
        for file in os.listdir(directory):
            if file.endswith('.json'):
                self.json_listbox.insert('', 'end', text=file)
            elif file.endswith('.txt'):
                self.txt_listbox.insert('', 'end', text=file)
        
        self.refresh_view_lists()

    def refresh_view_lists(self):
        # Clear existing items
        self.view_json_listbox.delete(*self.view_json_listbox.get_children())
        self.view_txt_listbox.delete(*self.view_txt_listbox.get_children())
        
        directory = self.dir_entry.get()
        
        for file in os.listdir(directory):
            if file.endswith('.json'):
                self.view_json_listbox.insert('', 'end', text=file)
            elif file.endswith('.txt'):
                self.view_txt_listbox.insert('', 'end', text=file)

    def on_file_select(self, event):
        # Clear current content
        self.content_text.delete(1.0, END)
        
        # Get selected item from either listbox
        widget = event.widget
        ...
        filename = widget.item(selection[0])['text']
    ...
    def search_documents(self):
        ...
        selected_json = self.json_listbox.selection()
        selected_txt = self.txt_listbox.selection()
        
        directory = self.dir_entry.get()
        results = []

        for item in selected_json:
            filename = self.json_listbox.item(item)['text']
            filepath = os.path.join(directory, filename)
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    for page in data['pages']:
                        if search_term in page['content'].lower():
                            results.append(f"Arquivo: {filename}\nPágina: {page['page_number']}\n"
                                        f"Conteúdo: {page['content']}\n{'-'*50}\n")
            except Exception as e:
                self.results_text.insert(END, f"Erro ao ler {filename}: {str(e)}\n")

        for item in selected_txt:
            filename = self.txt_listbox.item(item)['text']
            filepath = os.path.join(directory, filename)
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    content = f.read()
        ...
        if results:
            ...
            self.results_text.insert(END, "Nenhum resultado encontrado.\n")
